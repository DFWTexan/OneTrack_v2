...
import { AdminComService, AdminDataService, DropdownDataService, ErrorMessageService, UserAcctInfoDataService } from '../../../_services';
...
export class EditXborRequirementComponent implements OnInit, OnDestroy {
      ...
      this.adminComService.modes.xborLicenseRequirement.changed.subscribe((mode: string) => {
        if (mode === 'EDIT') {
          ...
          this.adminDataService.xborLicenseRequirementChanged.subscribe((xborLicenseRequirement: any) => {
            this.isStartDocUploaded = xborLicenseRequirement.startDocument ? true : false;
                this.isRenewalDocUploaded = xborLicenseRequirement.renewalDocument ? true : false;
  ...
  onSubmit(): void {
    ...
    let stateReqItem: any = this.xborLicenseRequirementForm.value;
